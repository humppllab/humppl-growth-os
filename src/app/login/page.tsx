...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err)
      setError(err.message || "Invalid credentials. Try admin@humppl.com / admin.")
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      ...
      console.error(err)
      setError("Demo login failed. Please try credentials manually.")
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-slate-900 overflow-hidden select-none z-[9999]">
      ...
      <div className="w-full max-w-md px-4 relative z-10">
        ...
        <div className="text-center mb-8 space-y-2 animate-in fade-in slide-in-from-top-4 duration-500">
          ...
          <p className="text-sm text-slate-400 font-medium">Internal CRM & Workflows Portal</p>
        </div>
        ...
        <Card className="border-slate-800 bg-slate-900/60 backdrop-blur-xl shadow-2xl rounded-2xl overflow-hidden animate-in fade-in zoom-in duration-500">
          <CardContent className="p-8">
            <form onSubmit={handleLogin} className="space-y-5">
              {error && (
                <div className="p-3 text-xs bg-rose-500/10 border border-rose-500/20 rounded-xl text-rose-400 font-medium text-center">
                  {error}
                </div>
              )}
              ...
              <div className="pt-2">
                <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-xl py-2.5 font-semibold text-sm transition-all" disabled={loading}>
                  {loading ? (
                    <>
        ...
        <div className="text-center mt-6">
          <p className="text-xs text-slate-600 font-medium">Use demo mode or admin@humppl.com / admin</p>
        </div>
      </div>
    </div>
  )
